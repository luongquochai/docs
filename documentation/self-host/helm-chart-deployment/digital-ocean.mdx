...
title: Digital Ocean
description: Install and configure API Lab on Digital Ocean using Helm Charts for cloud-based deployment.
---
...
With DigitalOcean's managed Kubernetes service and Helm charts, deploying API Lab on your infrastructure becomes a plain sailing process.
...
To deploy API Lab on DigitalOcean, you will need a DigitalOcean account. If you don't have an account already, sign up for one [here](https://www.digitalocean.com/).
...
#### **Helm `version 3.x`**
    Helm is a package manager for Kubernetes, which simplifies the deployment and management of applications like API Lab. Helm uses **charts**, which are pre-configured application definitions, to deploy applications to Kubernetes clusters. To install Helm, follow the [Helm installation guide](https://helm.sh/docs/intro/install/).

## Installation and Deployment

Follow the steps below to deploy API Lab on your DigitalOcean Kubernetes cluster:
...
<Steps>
  <Step title="Digital Ocean Cluster Connection">
    ...
    3. Select the Kubernetes cluster that you want to deploy API Lab on.
  ...
  <Step title="Install NGINX Ingress Controller (Optional)">
    If you need to expose API Lab to external traffic via an `HTTP/S` interface, you will need an Ingress Controller. [NGINX](https://nginx.org/) is a popular choice for this.
  ...
  <Step title="Configure and Prepare the 'values.yaml' File">
    ...
    # If using API Lab Enterprise, configure your license key
    enterprise:
      licenseKey: "1e8xxxxx-fexx-49xx-8cxx-d57f54fxxxxx"
    
    ```

    <Note> If you are deploying API Lab Enterprise edition, you'll need a valid license key. To obtain one, visit the [API Lab Enterprise Store](https://enterprise.hoppscotch.com/) and make a purchase. After completing the purchase, you'll receive an email with the license key. Alternatively, you can find the key in your API Lab Enterprise Dashboard. For more details on managing your license key, refer to this [guide](/guides/articles/manage-an-enterprise-license-key). </Note>
  </Step>
  <Step title="Deploy API Lab on Kubernetes">
    Now that you've configured the `values.yaml` file, you're ready to deploy API Lab using Helm.
...
After deploying the application, verify that everything is running smoothly by checking the status of your API Lab pods and inspecting the service details, including the external IP or LoadBalancer that you'll use to access the application.

```bash
# List all the pods running in the cluster
kubectl get pods

# View services to ensure that API Lab is exposed properly
kubectl get services
```

## Setup and Access
After successfully deploying API Lab and verifying the installation, the next step is to set up the Admin Dashboard and then access the API Lab application for API development and testing.
...
### Access the API Lab app
With the administrator account set up, you can now start using the API Lab app:
1. Open a new browser tab and visit `https://frontend.yourdomain.com`.
2. Begin developing and testing your APIs seamlessly with API Lab.
...
To uninstall API Lab and clean up the resources, you can remove both community and enterprise editions: 
