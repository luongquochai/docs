...
title: Understanding the GraphQL Schema on API Lab
description: Learn how to use the GraphQL schema on API Lab.
---

The GraphQL schema is at the core of any GraphQL server. It defines the capabilities of the API and specifies how clients can access that data. A schema includes types and fields, queries and mutations, interfaces, and unions. API Lab provides you the functionality to connect to a GraphQL server and explore the GraphQL schema.
...
API Lab's GraphQL API platform provides you with the best experience to test and play around with GraphQL. Once connected to a GraphQL server, you can view the schema by clicking the schema icon on the sidebar.

For example, try connecting to the following server

```
https://echo.app.apilab.work/graphql
```
