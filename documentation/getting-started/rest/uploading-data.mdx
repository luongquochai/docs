...
description: Learn how to upload data to an API using API Lab.
---
...
Let's take a look at **uploading an image** file to an API using API Lab:
...
When sending multiple types of data in a single request using `multipart/form-data`, you can assign a specific content type to each parameter in API Lab. Here's how:
...
<Note> Currently, the **API Lab Desktop App** does **NOT** support uploading **Binary files** directly as request bodies for APIs. </Note>