---
sidebarTitle: Self-Host API Lab
title: Self-Host API Lab on your own servers
description: Set up API Lab on your servers for complete control and customization.
---

Self-hosting API Lab gives you complete control over your API development workflow and allows you to deploy API Lab in your own data center or cloud, giving you greater control over data and security.

Self-hosted API Lab comes in two variants - [Community](https://docs.app.apilab.work/documentation/self-host/community-edition/getting-started) and [Enterprise Edition](https://docs.app.apilab.work/documentation/self-host/enterprise-edition/getting-started), both of which can be deployed on systems that support Docker. You can host API Lab on your servers, providing a private workspace for the individuals or teams using it.

This guide covers the basics of self-hosting API Lab, including the configurations and settings needed to get started.
...
- [Docker](https://docs.docker.com/engine/install) (v20+) - Docker should be properly installed and configured for containerization of your API Lab instance.
- [Git](https://git-scm.com/download) - Required for version control and managing API Lab source code.

Visit our [documentation](https://docs.app.apilab.work/documentation/self-host/community-edition/prerequisites) for a detailed guide on installing the prerequisite softwares.
...
API Lab uses a Postgres database to store all the data. You can use any Postgres database provider of your choice, hosted locally or on a cloud.
...
To invite your team to use API Lab and enable email delivery, you'll need to configure SMTP settings properly. 
...
To access the admin dashboard, you'll need to configure an OAuth provider. In the Community Edition, API Lab supports:
...
When `ENABLE_SUBPATH_BASED_ACCESS=true`, you can access all three services (API Lab App, Admin Dashboard, API Lab Backend) on the same domain using different routes. If subpath access is disabled **(`ENABLE_SUBPATH_BASED_ACCESS=false`)**,  you will need to access the services on different ports.
...
Once the environment variables are configured, you may now proceed to the next step of setting up the API Lab instance. 
<Steps>
  ...
  <Step title="Select the suitable API Lab instance to self-host">
    There are two ways to set up API Lab:
    1. **Using individual containers for the services** - API Lab Backend, API Lab Frontend and API Lab Admin Dashboard.
    2. **Using the AIO container** - a single container that provides all the services required to run API Lab.
    
    For a streamlined setup, let's proceed with the AIO container. If you'd like to set up individual containers instead, [refer to the documentation](https://docs.app.apilab.work/documentation/self-host/community-edition/install-and-build#using-individual-containers-for-the-services).
  </Step>
  <Step title="Pull the latest API Lab container">
  ...
  <Step title="Start the API Lab Instance">
