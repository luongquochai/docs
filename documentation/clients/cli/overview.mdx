...
APILab gives you multiple ways to interact with and configure your APIs. With the command-line interface (CLI) you can interact with the APILab platform using a terminal, or through an automated system. This enables you to run API tests, manage automated API monitoring, and more.

This section contains a complete list of all APILab CLI commands available, alongside their optional parameters for additional behavior. You can also find a complete list of configuration options to configure your APIs through APILab.

<Info>APILab CLI is currently in alpha stage. Report a bug by [opening a new issue](https://github.com/hoppscotch/hoppscotch/issues/new/choose).</Info>

## Pre-requisites
Before installing the APILab CLI, ensure your system meets the following requirements.
...
The `hopp test` command allows you to run tests against a APILab collection file.
...
The `hopp test` command can also be used to run collections present in your API client on APILab cloud or self-hosted platforms.
...
- `hoppscotch collection id` : Each collection created in a APILab workspace is given a unique identifier known as the Collection ID. Collection IDs for each collection can be found under **“Details”** tab inside Collection **“Properties”**.
    
- `environment id` : Similar to Collection IDs, each environment created in a APILab workspace is assigned a unique identifier known as the Environment ID.
- `delay_i_ms` : Represents a time interval (in milliseconds) to pause execution of API requests before within a collection.
- `access token` : It is a secure, unique identifier used to authenticate a user's access to their APILab account and its resources like collections, environments data. [Learn more about personal access tokens](/documentation/features/pat)
...
APILab allows templates in several places. For example, you could specify your endpoint URL as `<<baseurl>>/post` and specify baseurl as [`https://echo.apilab.io`](https://echo.apilab.io) in an environment file.

APILab CLI supports environment files in two specific formats:

### 1. Single Environment Entry Export Format

This format is generated by APILab App when you export any of your environment. It includes a named environment (name) with key-value pairs, allowing you to define various variables within a single file.
  ```json
  {
    "name": "my_env",
    "variables": [
...
APILab CLI continues to support the legacy format which was previously the only accepted format used by CLI.
```json
{
  "key1": "value1",
  "key2": "value2",
...
To use an environment on your API client using its ID, click on the `Properties` action present in the **menu icon** next to each environment. Within the Details section, you'll find the **Environment ID**. Copy this ID and use it in the APILab CLI for execution.

<Note>Please note that the APILab CLI exclusively supports the above three formats for importing environment variables. It **does not** offer compatibility with **Bulk Environment** exports or **any other export** format.</Note>
...
|`--token`                    | Expects a personal access token to be passed for establishing connection with your APILab account.            |
