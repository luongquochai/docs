...
description: If an update causes issues with API Lab Desktop, you can downgrade to a previous version and restore your data from an automatic backup. This guide will walk you through the entire process safely.
---

If an update causes issues with API Lab Desktop, you can downgrade to a previous version and restore your data from an automatic backup. This guide will walk you through the entire process safely.

## Understanding automatic backups

API Lab Desktop automatically creates backups when you first launch a new version. These backups are stored in version-specific folders, making it easy to identify which backup corresponds to which app version.
...
Visit the [API Lab releases page](https://github.com/hoppscotch/releases) and download the version you want to restore to.
...
Before downgrading, it's recommended to log out of your API Lab account to prevent sync conflicts during the restoration process. This makes sure that the cloud sync doesn't try to merge or overwrite data during the transition.

1. Open API Lab Desktop
2. Click on your profile icon in the top right corner
3. Select "Log out"
4. Confirm the logout

### Step 4: Close API Lab Desktop completely

Make sure API Lab Desktop is not running before proceeding:
...
**macOS:**
1. Open the `.dmg` file
2. Drag API Lab to Applications (replace if prompted)
...
Launch API Lab Desktop and verify that everything is working correctly:
1. Open API Lab Desktop
