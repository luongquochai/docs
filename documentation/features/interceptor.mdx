...
<Tabs>
  <Tab title="API Lab Agent">
    ...
    2. Open [API Lab](https://app.apilab.work) app and navigate to the "**Interceptors**" section in the **"Settings"** page.
    3. Locate the "**Agent**" option within the Interceptors section to initiate a connection to your local agent.
    4. The application will prompt you for a "**One-Time Verification Code**". This verification code will be generated within the API Lab Agent.
  ...
  <Tab title="Proxy">
    ...
    - Enable proxy interceptor from "[**Settings page**](https://app.apilab.work/settings)" under the **Interceptors** section.
    ...
    Once the container is running or the server is active, configure API Lab to use `http://localhost:9159` as the Proxy URL in the "**Settings**" page.
  ...
  <Tab title="Browser Extension">
    ...
    2. Click on the API Lab Browser Extension icon from the browser toolbar and ensure that `app.apilab.work` is in your active origins. If you are using API Lab Self-Host, add your own domain as a new origin
